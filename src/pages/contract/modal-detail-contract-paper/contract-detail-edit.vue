                ...
                >
                        ...
                        <div class="form-group row" slot-scope="{ errors }">
                          ...
                          <div class="col-sm-9">
                            <datepicker
                              ...
                              :language="vi"
                              v-model="dataEdit.effectiveFrom"
                            ></datepicker>
                          ...
                          <div class="table-responsive">
                            ...
                            >
                              ...
                              <tbody>
                                <tr
                                  class="table-row"
                                  v-for="(item, index) in dataEdit.deviceInfos"
                                  :key="index"
                                >
                  ...
                  <div>
                    ...
                    <div class="form-group row img-list">
                      <ul>
                        <li v-for="(item, index) in imageDataCurrent" :key="index">
...
export default {
  props: ['showModalEdit', 'datadepositType', 'dataConfigTypeContract', 'devices', 'codeContract', 'dataEdit'],
  components: {
    Datepicker
  },
  ...
  created () {
    this.$parent.$on('successEdit', this.uploadImg(this.dataEdit.code))
    this.imageDataCurrent = this.dataEdit.images ? this.dataEdit.images : []
  },
  ...
  methods: {
    ...
    async onSelectFile () {
      ...
      if (files && files[0]) {
        ...
        reader.onload = (e) => {
          ...
          this.imageDataCurrent.push(e.target.result)
        }
    ...
    onChangeCount (price, quantity, index) {
      if (price || quantity) {
        this.dataEdit.deviceInfos[index].price = price
        this.dataEdit.deviceInfos[index].totalPrice = quantity * price
      }
    },
    ...
    async submitEdit () {
      try {
        ...
        this.$emit('submitEdit', this.dataEdit)
      } catch (error) {
        console.log(error)
      }
    },
      ...
      if (this.arrayFormData.length > 0) {
        ...
        if (this.arrayImgInsertSuccess.length > 0) {
          ...
          await ContractSevice.saveImg({
            ...
            images: this.arrayImgInsertSuccess.concat(this.imageData)
          })
        }
      }
    },
    ...
    removeDevice (index) {
      this.dataEdit.deviceInfos.splice(index, 1)
    },
