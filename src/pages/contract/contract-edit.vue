<template>
  <div class="addContract">
    ...
    <div class="card p-10 pt-15 b-t-1-869fb229">
      <div
        class="card-body px-20 row"
        v-if="dataInsert && dataInsert.state === 'REJECT'"
      >
        <ValidationObserver ref="contractAdd" class="card-body px-20 mt-2 row">
          <form class="col-md-6 mb-3">
            ...
            >
              <div class="form-group row" slot-scope="{ errors }">
                ...
                <div class="col-sm-8">
                  <input
                    :disabled="dataInsert.state === 'REJECT'"
                    type="text"
                    class="form-control f12"
                    id=""
                    placeholder=""
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-8">
                <input
                  ...
                  class="form-control f12"
                  :value="
                    (dataInsert.customerInfo
                      ? dataInsert.customerInfo.address
                      : '') +
                    ' ' +
                    (dataInsert.customerInfo
                      ? dataInsert.customerInfo.district
                      : '') +
                    ' ' +
                    (dataInsert.customerInfo
                      ? dataInsert.customerInfo.city
                      : '')
                  "
                  placeholder=""
                  disabled
          ...
          <form class="col-md-6 mb-3">
            ...
            <div class="form-group row">
              <label for="" class="col-sm-4 col-form-label">Ngày ký:</label>
              <div class="col-sm-8">
                <datepicker
      ...
      <div
        class="card-body px-20 mt-5 row"
        v-if="
          dataInsert &&
          (dataInsert.state === 'REJECT' ||
            dataInsert.state === 'NEW' ||
            dataInsert.state === 'WAITING_APPROVED')
        "
      >
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body text-center">
                    ...
                    <h2 class="f18 py-3">Cập nhật thành công</h2>
                  </div>
                  <div class="modal-footer">
                    <button
                      type="button"
...
export default {
  ...
  methods: {
    async fetchData (code) {
      const data = await ContractSevice.getByCode({
        code: code
      })
      if (data.code === 1000) {
        this.dataInsert = data.data
        this.objectMerchantRaw = data.data
        this.onChangSelectService(this.dataInsert.service)
        this.dataDevice = this.dataInsert.deviceInfos
        console.log(this.dataInsert)
        this.imageData = this.dataInsert.images
          ? this.dataInsert.deviceInfos
          : []
        this.objectMerchant = this.dataInsert.customerInfo
          ? this.dataInsert.customerInfo
          : {}
      }
    },
    ...
    async addContract () {
      ...
      try {
        if (this.dataInsert.state === 'REJECT') {
          ...
          if (
            (this.objectMerchant.mobile &&
              this.objectMerchant.mobile !==
                this.objectMerchantRaw.customerInfo.mobil) ||
            (this.objectMerchant.email &&
              this.objectMerchantRaw.customerInfo !== this.objectMerchant.email)
          ) {
            const body = {
        ...
        if (
          this.dataInsert.state === 'WAITING_APPROVED' ||
          this.dataInsert.state === 'NEW'
        ) {
          await this.uploadImg(this.dataInsert.code)
          this.showModalSuccess = true
          return
        }
      } catch (error) {
